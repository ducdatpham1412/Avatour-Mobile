...
const CommentReplies = (props: Props) => {
  ...
  const totalRemains = displayList ? totalReplies - list.length : totalReplies;
  const listHeight = displayList ? undefined : 0;
  const showButtonReplies = totalRemains > 0 || !displayList;

  useEffect(() => {
    if (
      bubblePalaceAction.action === TYPE_BUBBLE_PALACE_ACTION.addReplyComment &&
  ...
  const RenderItemComment = useCallback(
    (item: TypeCommentResponse) => {
      return (
        <ItemComment item={item} onPressReply={onPressReply} postId={postId} />
      );
    },
    [postId],
  );
          ...
          {showButtonReplies ? (
            <StyleTouchable
              ...
              <StyleText
                ...
                style={[styles.textViewReplies, {color: theme.borderColor}]}
              />
              <AntDesign
                name="down"
