...
const TitleInformation = (props: ParamsInformation) => {
  ...
  return (
    <View style={styles.informationView}>
      ...
      <StyleText
        i18Text={title}
        customStyle={[styles.titleInformation, {color: titleColor}, titleStyle]}
      />
    </View>
  );
};
...
const DetailGroupBuying = ({route}: Props) => {
  ...
  const isInRoot = useRef(route.name === ROOT_SCREEN.detailGroupBuying).current;
  ...
  useEffect(() => {
    if (
      bubblePalaceAction.action === TYPE_BUBBLE_PALACE_ACTION.editGroupBuying
    ) {
      ...
      if (!isMyBubble && item.amount !== null && item.deposit !== null) {
        return (
          <View style={styles.depositView}>
            <View style={[styles.depositBox, {borderColor: theme.borderColor}]}>
              {item.amount && (
                <StyleText
              ...
              {!!item.note && (
                <StyleText
                  ...
                  customStyle={[styles.textDeposit, {color: theme.borderColor}]}
                />
              )}
            </View>
          </View>
      ...
      <View style={styles.infoPart}>
        ...
        {displayDontWorry && (
          <View style={styles.pricesView}>
            <StyleText
              ...
              style={[styles.peoplePriceText, {color: theme.highlightColor}]}
            />
          </View>
        )}
        ...
        <View style={styles.pricesView}>
          <View style={styles.priceBox}>
            <StyleText
              originValue={`${formatLocaleNumber(item.retailPrice)} vnd`}
              style={[styles.peoplePriceText, {color: theme.highlightColor}]}
            />
          </View>
        </View>
        ...
        <View style={styles.pricesView}>
          {item.prices.map((price, index) => (
            <View key={index} style={[styles.priceBox, {marginBottom: 5}]}>
              <StyleText
        ...
        <View style={styles.informationView}>
          <Ionicons
            ...
            style={[styles.iconLocation, {color: Theme.common.commentGreen}]}
          />
          <StyleText
  ...
  const ToolReaction = () => {
    ...
    return (
      <View style={[styles.reactionView, {borderTopColor: theme.holderColor}]}>
        <View style={styles.starBox}>
          {isLiked ? (
            <IconLiked
              customStyle={[styles.star, {color: theme.likeHeart}]}
              onPress={() =>
                onHandleLike({
                  isModeExp,
                  isLiked,
            ...
            <IconNotLiked
              customStyle={[styles.star, {color: theme.unLikeHeart}]}
              onPress={() =>
                onHandleLike({
                  isModeExp,
                  isLiked,
          ...
          <StyleText
            i18Text={totalLikes ? 'discovery.numberLike' : 'discovery.like'}
            i18Params={{
              value: 23,
            }}
            customStyle={[styles.textStar, {color: theme.borderColor}]}
        ...
        <View style={styles.starBox}>
          ...
          <StyleText
            i18Text={
              totalComments ? 'discovery.numberComments' : 'discovery.comment'
            }
            i18Params={{
    ...
    return (
      <>
        {displayText && (
          <StyleText
            i18Text={
              3 ? 'discovery.numberGroupJoined' : 'discovery.beTheFirstJoin'
            }
            customStyle={[
        ...
        <StyleTouchable
          ...
          onPress={() => modalJoinedRef.current?.open()}>
          {fakeDataListGroupsJoined[0].listPeople.map(
            (peopleJoin: TypePeopleJoinedResponse, index: number) => (
              <StyleImage
                key={index}
          ...
          {isBiggerThanSix && (
            <View
              ...
              />
              <StyleText
                originValue={`+${item.totalGroups - displayPeople.length}`}
                customStyle={[
                  styles.textJoinMore,
                  {color: theme.textHightLight},
                ]}
  ...
  const ListPeopleRetail = () => {
    ...
    const isBiggerThanSix = item.totalGroups > 6;
    const displayPeople: Array<TypePeopleJoinedResponse> = [];
    listJoinPersonalPaging.list.every((personal: TypePeopleJoinedResponse) => {
      if (displayPeople.length < 6) {
  ...
  const Content = () => {
    return (
      <View style={[styles.contentBox, {borderTopColor: theme.holderColor}]}>
        <View style={styles.creatorAvatarTouch}>
          ...
          <View style={styles.nameTimeTouch}>
            <StyleText
              ...
              customStyle={[styles.creatorNameText, {color: theme.textColor}]}
              numberOfLines={1}
              onPress={() => onGoToProfile(item.creator)}
            />
            <StyleText
              originValue={formatFromNow(item.created)}
              customStyle={[styles.timeText, {color: theme.borderColor}]}
            />
          </View>
        </View>
        <StyleText
    ...
    <>
      <ScrollView
        ...
        <SharedElement
          ...
          <ScrollSyncSizeImage
            images={[
              'https://www.vietnambooking.com/wp-content/uploads/2020/05/tour-ha-giang-2-ngay-3-dem-4.jpg',
            ]}
            syncWidth={width}
            onDoublePress={() => {
              if (!isLiked) {
