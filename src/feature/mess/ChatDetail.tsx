...
const ChatDetail = ({route}: ChatDetailProps) => {
  ...
  const [modalPickImgHeight, setModalPickImgHeight] = useState(initHeightModal);
  ...
  const RenderTyping = useMemo(() => {
    ...
    if (isMyChatTag) {
      userIdDisplay = itemChatTag.userTyping.find(item => item === profile.id);
      if (!userIdDisplay) {
        return null;
      }
    } else {
    ...
    const avatarDisplay =
      itemChatTag.listUser.find(item => item.id === userIdDisplay)?.avatar ||
      '';
