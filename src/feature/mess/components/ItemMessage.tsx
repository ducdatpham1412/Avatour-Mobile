...
const ItemMessage = (props: Props) => {
  ...
  const displayAvatar = isMyMessage ? displaySeenAvatar : displayPartnerAvatar;
      ...
      <View
        ...
        {isMyMessage && isDisplayingOption && (
          <View style={styles.optionsBox}>
            <StyleTouchable onPress={() => onDeleteMessage(itemMessage.id)}>
        ...
        >
          {isMessageImage && (
            <View style={styles.imageBox}>
              {itemMessage.content?.map((item: string, index: number) => (
                <MessageImage
                  key={index}
                  imageUrl={item}
          ...
          {isMessageEmoji && (
            <StyleTouchable
              activeOpacity={1}
              normalOpacity={isDisplayDatetime && mostHeightDateTime ? 0.6 : 1}
          ...
          <Text style={styles.comeBackButtonBox} onPress={Keyboard.dismiss} />
        </Animated.View>
      </View>
