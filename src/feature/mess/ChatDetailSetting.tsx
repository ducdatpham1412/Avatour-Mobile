import {apiChangeChatColor, apiChangeChatName} from 'api/conversation';
import {
  apiBlockUser,
  apiOpenConversation,
  apiStopConversation,
...
const ChatDetailSetting = ({route}: Props) => {
  ...
  const partnerInfo = useMemo(() => {
    const result = itemChatTag.listUser.find(item => item.id !== profile.id);
    return (
      result || {
        id: profile.id,
        name: profile.name,
  ...
  const IconConversationName = () => {
    return (
      <Ionicons
        name="text"
        style={[styles.textConversationName, {color: theme.textHightLight}]}
      />
    );
  };
  ...
  const IconBlock = () => {
    return (
      <Entypo
        name="block"
        style={[styles.textConversationName, {color: theme.highlightColor}]}
      />
    );
  };
          ...
          <View
            ...
            <StyleTouchable
              ...
              <StyleText
                ...
                customStyle={[styles.textButton, {color: theme.highlightColor}]}
              />
            </StyleTouchable>
            <StyleTouchable customStyle={styles.buttonBox} onPress={onSave}>
              <StyleText
                i18Text="common.save"
                customStyle={[
                  styles.textButton,
  ...
  return (
    <View style={[styles.container, {backgroundColor: theme.backgroundColor}]}>
      <StyleHeader
        title={itemChatTag.conversationName || 'mess.detailSetting.title'}
        titleStyle={{width: '60%'}}
      />
        ...
        {!isChatTagOfMe && (
          <View
            ...
            <ItemSettingSwitch
              ...
              value={itemChatTag.status === CONVERSATION_STATUS.stop}
