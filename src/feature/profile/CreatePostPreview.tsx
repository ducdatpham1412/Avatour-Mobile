import {TypeCreatePostRequest, TypeEditPostRequest} from 'api/interface';
import {apiCreatePost, apiEditPost} from 'api/profile';
...
const CreatePostPreview = ({
  ...
  const initValue = useMemo(() => {
    return {
      ...
      feeling: itemEdit?.feeling || itemError?.feeling || itemDraft?.feeling,
      location:
        itemEdit?.location || itemError?.location || itemDraft?.location,
      ...
      <View style={styles.feelingTopicLocationView}>
        {(!!chooseFeeling || !!topics.length) && (
          <View style={styles.locationBox}>
            ...
            {topics.map(item => {
              const chooseTopic = LIST_TOPICS.find(_to => _to.id === item);
              if (chooseTopic) {
                return (
                  <>
                    <StyleText
        ...
        {!!location && (
          <View style={styles.locationBox}>
            ...
            <StyleText
              ...
              customStyle={[styles.textLocation, {color: theme.textColor}]}
            />
          </View>
        )}
      </View>
  ...
  const StarLink = () => {
    const Link = () => {
      ...
      return (
        <StyleTouchable
          customStyle={[styles.addLinkBox, {borderColor: theme.textColor}]}
          onPress={onAddLink}>
          <AntDesign
            name="plus"
      ...
      <View style={styles.starLinkView}>
        <Animated.View
          style={[
            styles.starBox,
            {
              transform: [{scale: scaleStars}, {translateX: translateXStars}],
              backgroundColor: theme.backgroundButtonColor,
            },
          ]}>
          <StyleTouchable onPress={onChooseStar} customStyle={styles.starTouch}>
            {NUMBER_STARS.map((_, index) => {
              ...
              return (
                <AntDesign
                  key={index}
                  name="staro"
                  style={[styles.star, {color: Theme.common.white}]}
                />
              );
            })}
            <StyleText
    ...
    <View style={[styles.container, {backgroundColor: theme.backgroundColor}]}>
      ...
      <Animated.View
        ...
        <View style={styles.buttonToolBox}>
          ...
          <View style={[styles.divider, {backgroundColor: theme.textColor}]} />
          <StyleTouchable
            customStyle={styles.buttonStarTool}
            onPress={onFeeling}>
            {IconEmotion}
          </StyleTouchable>
          <StyleTouchable customStyle={styles.buttonStarTool} onPress={onTopic}>
            {IconTag}
          </StyleTouchable>
          <StyleTouchable
            customStyle={styles.buttonStarTool}
