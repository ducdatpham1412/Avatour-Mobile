...
interface Props {
  route: {
    params: AppParamsList[ROOT_SCREEN.editHistory];
  };
}
...
const EditHistory = ({route}: Props) => {
  ...
  const RenderItem = useCallback(
    (item: TypeEditPriceHistory) => {
      return (
        <View style={[styles.itemContainer, {borderColor: theme.borderColor}]}>
          <StyleText
            originValue={formatDayGroupBuying(item.created)}
            customStyle={[styles.textDatetime, {color: theme.borderColor}]}
          />
          <StyleText
            i18Text="discovery.retailPrice"
            customStyle={[styles.textRetail, {color: theme.textHightLight}]}>
            <StyleText
