import {apiGetListPostsArchived} from 'api/profile';
...
const PostsArchived = () => {
  ...
  useEffect(() => {
    if (bubblePalaceAction.action === TYPE_BUBBLE_PALACE_ACTION.unArchivePost) {
  ...
  const RenderItem = useCallback((item: TypeBubblePalace & TypeGroupBuying) => {
    return (
      <PostStatus
        item={item}
