...
interface Props {
  route: {
    params: AppParamsList[PROFILE_ROUTE.updatePrices];
  };
}
  ...
  const Notification = () => {
    return (
      <StyleText
        ...
        customStyle={[styles.notiChangePrice, {color: theme.borderColor}]}
      />
    );
  };
    ...
    <View style={[styles.container, {backgroundColor: theme.backgroundColor}]}>
      ...
      <View style={{flex: 1}}>
        <ScrollView contentContainerStyle={styles.contentContainer}>
          ...
          <StyleTouchable
            customStyle={[styles.inputView, {borderColor: theme.borderColor}]}
            onPress={() => modalRetailRef.current?.show()}>
            <StyleText
              originValue={`${formatLocaleNumber(item.retailPrice)} vnd`}
              customStyle={[styles.textRetailPrice, {color: theme.textColor}]}
            />
          </StyleTouchable>
