  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const temp = data.find((item: any) => item?.code === setting.bank_code);
        if (temp) {
          setChosenBank(temp);
        }
      } catch (err) {
  ...
  return (
    <View style={[styles.container, {backgroundColor: theme.backgroundColor}]}>
      ...
      <StyleText
        ...
        customStyle={[styles.titleChooseBank, {color: theme.textHightLight}]}
      />
      <View style={styles.infoView}>
        ...
        <StyleTouchable onPress={() => modalChooseBankRef.current?.show()}>
          <StyleText
            ...
            customStyle={[styles.textEdit, {color: theme.borderColor}]}
          />
        </StyleTouchable>
      </View>
      ...
      <StyleText
        ...
        customStyle={[styles.titleAccountNumber, {color: theme.textHightLight}]}
      />
      <View style={styles.infoView}>
        <View style={[styles.infoBox, {borderColor: theme.borderColor}]}>
          {bankAccount ? (
            <StyleText
              ...
              customStyle={[styles.textNotYet, {color: theme.borderColor}]}
            />
          ) : (
            <StyleText
              i18Text="setting.personalInfo.notYet"
        ...
        <StyleTouchable onPress={() => modalBankAccountRef.current?.show()}>
          <StyleText
            i18Text="profile.post.edit"
            customStyle={[styles.textEdit, {color: theme.borderColor}]}
          />
